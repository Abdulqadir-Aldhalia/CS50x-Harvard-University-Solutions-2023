...
void encrypt(char **node)
{
    ...
    char *new_str = malloc(strlen(word) + 1); // allocate memory for word and null terminator
    if (new_str == NULL)
    {
    ...
    {
        // copy word into new memory
        strcpy(new_str, word); // or memcpy(new_str, word, strlen(word) + 1);
...
void decrypt (char **node)
{
    ...
    //realloc memory for our node
    char *new_str = malloc(strlen(finalWord) + 1); // allocate memory for finalWord and null terminator
    if (new_str == NULL)
    {
        // handle allocation failure
        printf("failed in memory allocation");
    ...
    {
        // copy finalWord into new memory
        strcpy(new_str, finalWord); // or memcpy(new_str, finalWord, strlen(finalWord) + 1);
        // update the value that node points to with the new memory
        *node = new_str;
    }
    printf("\n");
